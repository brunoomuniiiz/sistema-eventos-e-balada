...
AS $function$
 ...
 BEGIN
   -- 1) Tenta Venda (Sales) por token ou código curto
   ...
   FROM public.sales s
   WHERE s.pickup_token = _token OR s.pickup_code = v_token
   LIMIT 1;
...
GRANT EXECUTE ON FUNCTION public.order_lookup_by_token(text) TO anon;
