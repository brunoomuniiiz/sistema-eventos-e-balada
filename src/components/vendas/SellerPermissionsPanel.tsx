  ...
  const { data: team = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        .from("user_roles")
        .select("id, user_id, display_name, email, role, permissions, aceita_dinheiro, aceita_pix, aceita_cartao, lojinha_can_sell, can_discount, max_discount_percent, vendas_pdv_caixa, vendas_garcom, vendas_validar_qr, vendas_pedidos, vendas_historico, vendas_fechamento, vendas_abre_caixa, vendas_sangria")
        .eq("owner_id", ownerId!)
