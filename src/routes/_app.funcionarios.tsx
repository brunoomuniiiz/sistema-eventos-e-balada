...
import { Plus, Trash2, UserCog, Crown, Pencil } from "lucide-react";
...
function FuncionariosPage() {
  const { user } = useAuth();
  const { isOwner, ownerId, can, loading } = usePermissions();
  ...
  if (!can("funcionarios")) {
    return <PageHeader title="Funcionários" subtitle="Você não tem permissão para acessar esta página" />;
  }
  ...
  return (
    <div>
      <PageHeader
        title="Funcionários"
        subtitle="Crie acessos com permissões específicas"
        actions={
          isOwner && (
